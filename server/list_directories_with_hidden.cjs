...
console.log('📚 开始列出所有目录（包含隐藏字符）...\n');

async function listAllDirectoriesWithHidden() {
  try {
    ...
    for (const entry of entries) {
      ...
      console.log(`   长度: ${entry.name.length}`);
      console.log(`   字符编码: ${Buffer.from(entry.name).toString('hex')}`);
      console.log(`   最后一个字符: ${entry.name[entry.name.length - 1]} (${entry.name.charCodeAt(entry.name.length - 1)})`);
      ...
      try {
        ...
        for (const subEntry of subEntries) {
          ...
          console.log(`     长度: ${subEntry.name.length}`);
          console.log(`     字符编码: ${Buffer.from(subEntry.name).toString('hex')}`);
          console.log(`     第一个字符: ${subEntry.name[0]} (${subEntry.name.charCodeAt(0)})`);
