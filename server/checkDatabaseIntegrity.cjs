...
function checkCalibreDatabase() {
  ...
  try {
    ...
    if (!hasBooks) {
      console.log('\n⚠️ 注意: 这是简化版 Calibre 数据库，不包含标准 Calibre 表结构');
      console.log('   这可能是正常的，取决于项目需求\n');
    }
...
function checkTalebookDatabase() {
  ...
  try {
    ...
    console.log(`📋 表数量: ${tableNames.length} 个\n`);
    console.log('--- 必需表检查 ---\n');
...
function checkQcbooklogDatabase() {
  ...
  const requiredFields = {
    ...
    'qc_bookmarks': ['id', 'mapping_id', 'content', 'created_at'],
...
console.log(`Calibre 数据库:   ${r1 ? '✅ 正常' : '⚠️ 需检查'}`);
console.log(`Talebook 数据库:  ${r2 ? '✅ 完整' : '❌ 不完整'}`);
console.log(`QCBookLog 数据库: ${r3 ? '✅ 完整' : '❌ 不完整'}`);
