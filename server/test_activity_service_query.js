...
const query = `
  ...
  FROM reading_state
  WHERE read_date IS NOT NULL AND DATE(read_date) >= DATE('${startDate}') AND DATE(read_date) <= DATE('${endDate}') ${readerFilter}
