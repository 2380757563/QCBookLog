/**
 * QCBookLog 书籍扩展数据仓储 (qc_bookdata 表)
 ...
 * QCBookLog 书籍扩展数据仓储类
 */
class QcBooklogQcBookdataRepository extends BaseRepository {
  /**
   * 根据书籍ID查找扩展数据
   */
  findByBookId(bookId) {
    ...
    try {
      ...
      if (existing) {
        ...
        if (data.paper1 !== undefined) {
          updates.push('paper1 = ?');
          values.push(data.paper1);
        }
        if (data.edge1 !== undefined) {
          updates.push('edge1 = ?');
          values.push(data.edge1);
        }
        if (data.edge2 !== undefined) {
          updates.push('edge2 = ?');
          values.push(data.edge2);
        }
...
export default QcBooklogQcBookdataRepository;