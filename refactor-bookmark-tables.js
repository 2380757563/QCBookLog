...
const insertTestData = () => {
  ...
  // 检查 books 表中是否有数据
  const books = db.prepare('SELECT id, title FROM books LIMIT 3').all();
  
  if (books.length === 0) {
    console.warn('⚠️  books 表中没有数据，跳过测试数据插入');
    return;
  }
  ...
  for (const bookmark of testBookmarks) {
    ...
    console.log(`✅ 插入书摘 ID: ${bookmarkId}, 书籍: ${books[0].title}`);
  }
...
const main = () => {
  try {
    ...
    console.log('     - id: 主键');
    console.log('     - book_id: 外键，关联 books 表');
